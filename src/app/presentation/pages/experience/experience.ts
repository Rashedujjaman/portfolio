...
export class Experience implements OnInit {
  ...
  private async loadData() {
    try {
      ...
      const [
        ...
        this.getProjectsUseCase.execute().toPromise(),
      ]);
      ...
      // Calculate skills with experience count from loaded projects (frontend processing)
      this.skillsWithExperience = this.calculateSkillsWithExperience(
        this.projects,
        this.allSkills
      );
    } catch (error) {
  ...
  private updateActiveTimeline() {
    const timelineItems =
      this.elementRef.nativeElement.querySelectorAll('.timeline-item');
  ...
  private updateParallaxEffect() {
    ...
    const parallaxElements =
      this.elementRef.nativeElement.querySelectorAll('.parallax-element');
  ...
  getEducationIcon(degree: string): string {
    ...
    if (
      ...
      return '📚';
    if (degreeType.includes('diploma') || degreeType.includes('certificate'))
      return '📜';
    return '🎓';
  }
  ...
  private calculateSkillsWithExperience(
    projects: Project[],
  ...
  getSkillStats(
    skill: string
  ): { skill: string; count: number; years?: number } | null {
  ...
  getSkillProgressPercentage(stats: {
    skill: string;
    count: number;
    years?: number;
  }): number {
